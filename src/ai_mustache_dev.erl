...
%%% loaded module is a compiled template, its -mustache_source attribute is
...
%% @doc Every loaded module that carries a -mustache_source attribute.
...
-spec source(module()) -> {ok, ai_mustache_source()} | {error, not_a_template_module}.
source(Mod) ->
    try Mod:module_info(attributes) of
        Attrs ->
            %% Attribute values arrive wrapped in a list by module_info/1.
            case lists:keyfind(mustache_source, 1, Attrs) of
                {mustache_source, [#{path := _, stamp := _} = S]} -> {ok, S};
                {mustache_source, #{path := _, stamp := _} = S}    -> {ok, S};
...
%% ai_mustache_compiler:source_hash/2 the plugin uses. Two independent
...
-spec stale_by_stamp(binary() | string(), binary(), ai_mustache_source()) ->
          boolean() | {error, term()}.
stale_by_stamp(Path, Stamp, #{opts := Opts}) ->
    case file:read_file(Path) of
        {error, R} -> {error, {R, Path}};
        {ok, Body} -> ai_mustache_compiler:source_hash(Body, Opts) =/= Stamp
    end.
...
%%
%% reload/1 recovers the options from the module's own -mustache_source, which
