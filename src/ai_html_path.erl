%%%-------------------------------------------------------------------
%%% @doc Template path resolution and static template-attribute scanning.
%%%
%%% Engine-neutral: the attribute name is a parameter of scan/2, so the
%%% mustache and jinja parse_transforms and the plugin all share one search
%%% order (designs/08-jinja-architecture.md section 1.1).
%%%
%%% Two consumers share this module and they must agree byte for byte:
%%%
%%% <ul>
%%%   <li>each engine's parse_transform resolves the path of every
%%%       template attribute it is about to compile;</li>
...
-export([resolve/2, template_spec/1, scan/2]).
...
%% @doc Read a template attribute term.
%%
...
%% Both shapes are used by every engine's template attribute.
...
%% @doc Read the template attributes named `Attr' out of an `.erl' source file.
%%
...
-spec scan(file:filename_all(), atom()) -> {ok, [template()]} | {error, term()}.
scan(File, Attr) ->
    case epp_dodger:quick_parse_file(File) of
        {ok, Forms} ->
            {ok, [T || {attribute, _, A, Term} <- Forms, A =:= Attr,
