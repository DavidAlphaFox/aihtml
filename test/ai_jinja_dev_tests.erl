...
%%% @end
%%%-------------------------------------------------------------------
-module(ai_jinja_dev_tests).

-include_lib("eunit/include/eunit.hrl").

-define(MOD, j2_dev_probe).
...
setup() ->
    Dir = filename:join(["/tmp", "aihtml_jinja_dev_" ++ integer_to_list(erlang:unique_integer([positive]))]),
...
write(Dir, Body) ->
    Path = filename:join(Dir, "probe.j2"),
...
t_source(Dir) ->
    Path = write(Dir, <<"hi {{ x }}">>),
    ok = build(Path, <<"hi {{ x }}">>),
    {ok, S} = ai_jinja_dev:source(?MOD),
    ?assertMatch(#{path := _, stamp := _, mtime := _, vsn := _, opts := _}, S),
    ?assertEqual(list_to_binary(Path), maps:get(path, S)).

t_not_template() ->
    ?assertEqual({error, not_a_template_module}, ai_jinja_dev:source(lists)).

t_fresh(Dir) ->
    Body = <<"hi {{ x }}">>,
    Path = write(Dir, Body),
    ok = build(Path, Body),
    ?assertEqual(false, ai_jinja_dev:stale(?MOD)).
...
t_edited(Dir) ->
    ...
    ok = build(Path, <<"hi {{ x }}">>),
    _ = write(Dir, <<"bye {{ x }}">>),
    ?assertEqual(true, ai_jinja_dev:stale(?MOD)).
...
t_reload(Dir) ->
    ...
    _ = write(Dir, <<"bye {{ x }}!">>),
    ok = ai_jinja_dev:reload(?MOD),
    ?assertEqual(<<"bye V!">>, ?MOD:render(#{x => <<"V">>})),
    ?assertEqual(false, ai_jinja_dev:stale(?MOD)).

%% The whole reason -jinja_source carries opts: without them reload/1 could
...
t_reload_opts(Dir) ->
    Path = write(Dir, <<"{{ x }}">>),
    ok = build(Path, <<"{{ x }}">>),
    {ok, #{opts := Opts}} = ai_jinja_dev:source(?MOD),
    ?assert(is_list(Opts)),
...
t_broken(Dir) ->
    Path = write(Dir, <<"{{ x }}">>),
    ok = build(Path, <<"{{ x }}">>),
    _ = write(Dir, <<"{% if a %}oops">>),
    ?assertMatch({error, {_File, _Line, {unclosed_block, 'if', _}}},
                 ai_jinja_dev:reload(?MOD)),
...
t_listing(Dir) ->
    Path = write(Dir, <<"{{ x }}">>),
    ok = build(Path, <<"{{ x }}">>),
    ?assert(lists:member(?MOD, ai_jinja_dev:template_modules())).
...
%% ai_html_dev is one function over two dev modules, and it must not start
%% anything of its own either.
html_dev_reload_all_test() ->
    Before = {length(ets:all()), length(erlang:processes())},
    ?assertEqual(ok, ai_html_dev:reload_all()),
    ?assertEqual(Before, {length(ets:all()), length(erlang:processes())}).

html_dev_check_test() ->
    ?assertEqual(ok, ai_html_dev:check()).
