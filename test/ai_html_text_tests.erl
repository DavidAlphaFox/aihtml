...
%%%
%%% The point of ai_html_text is that no module downstream has to ask what
...
template_accepts_binary_test() ->
    ?assertEqual({ok, <<"a{{x}}b">>}, ai_html_text:template(<<"a{{x}}b">>)).

template_accepts_string_test() ->
    ?assertEqual({ok, <<"abc">>}, ai_html_text:template("abc")).

template_accepts_iolist_test() ->
    ?assertEqual({ok, <<"abc">>}, ai_html_text:template([<<"a">>, "b", [<<"c">>]])).

template_keeps_utf8_test() ->
    ?assertEqual({ok, ?CJK}, ai_html_text:template(?CJK)).
...
template_rejects_invalid_utf8_test() ->
    ?assertEqual({error, {invalid_utf8, 3}}, ai_html_text:template(?LATIN1)).

template_reports_offset_test() ->
    ?assertMatch({error, {invalid_utf8, 0}},
                 ai_html_text:template(<<255, 254>>)).
...
path_normalises_test_() ->
    [?_assertEqual(<<"views">>, ai_html_text:path("views")),
     ?_assertEqual(<<"views">>, ai_html_text:path(<<"views">>)),
     ?_assertEqual(?CJK, ai_html_text:path(?CJK)),
     ?_assertEqual(?LATIN1, ai_html_text:path(?LATIN1))].
...
opts_leaves_absent_keys_absent_test() ->
    ?assertEqual(#{module => m}, ai_html_text:opts(#{module => m})).

source_defaults_test_() ->
    [?_assertEqual(<<"nofile">>, ai_html_text:source(#{})),
     ?_assertEqual(<<"x.mustache">>, ai_html_text:source(#{source => "x.mustache"}))].

to_list_round_trip_test_() ->
    [?_assertEqual("abc", ai_html_text:to_list(<<"abc">>)),
     ?_assertEqual(?CJK, unicode:characters_to_binary(
                           ai_html_text:to_list(?CJK), utf8))].
...
module_name_accepts_string_prefix_test() ->
    ...
    Opts = ai_html_text:opts(#{prefix => "view_"}),
