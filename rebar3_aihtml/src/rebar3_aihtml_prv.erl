%%%-------------------------------------------------------------------
%%% @doc The `rebar3 mustache' provider.
%%%
...
%% @doc Also the entry point for a caller that is not the command line.
run(State, Force) ->
    try
...
collect(State) ->
    lists:foldr(
      fun(App, Acc) ->
              case rebar3_aihtml_opts:for_app(App, State) of
                  skip ->
...
validate(Collected) ->
    ...
    ok = cross_app_warnings(Collected),
    case Errors of
        [] -> ok;
...
cross_app_warnings(Collected) ->
    Wae = lists:any(fun({_, O, _}) -> O#mopts.wae end, Collected),
    PerApp = [{Opts#mopts.app_name, Tpls} || {_, Opts, Tpls} <- Collected],
    case rebar3_aihtml_check:cross_app(PerApp) of
        [] -> ok;
        Dups ->
            Lines = [io_lib:format("~s is generated by apps ~p", [M, Apps])
                     || {M, Apps} <- Dups],
            Msg = lists:flatten(
                    ["module name(s) generated by more than one app; give them "
                     "different mustache_opts prefixes: " |
                     lists:join("; ", Lines)]),
            case Wae of
...
emit_app({_App, Opts, Tpls}, Force, {C0, S0, K0}) ->
    ...
    Touched > 0 andalso
        rebar_api:debug("mustache: touched ~b module(s) with a stale "
                        "-mustache_template", [Touched]),
    {C0 + Compiled, S0 + Skipped, K0 + Cleaned}.

one(T = #tpl{module = Mod, rel_path = Rel, out_path = Out, stamp = Stamp},
    Opts, Available, Force, Forced, {C, S, Errs}) ->
    case needs_build(Force, sets:is_element(Mod, Forced), Out, Stamp) of
        false ->
            rebar_api:debug("mustache: ~ts is up to date", [Rel]),
            {C, S + 1, Errs};
        true ->
            rebar_api:debug("mustache: compiling ~ts -> ~ts", [Rel, Out]),
...
needs_build(true, _Forced, _Out, _Stamp)  -> true;
needs_build(_Force, true, _Out, _Stamp)   -> true;
needs_build(_Force, _Forced, Out, Stamp)  ->
    ...
    rebar3_aihtml_scan:stamp_of(Out) =/= {ok, Stamp}.

report({0, 0, 0}) ->
    ok;
report({C, S, K}) ->
    rebar_api:info("mustache: compiled ~b, skipped ~b, cleaned ~b", [C, S, K]).
...
format_error({errors, Errors}) ->
    lists:flatten(
      lists:join("\n", ["mustache compilation failed:" |
                        [diagnostic(E) || E <- Errors]]));
format_error({bad_opts, Msg}) ->
    lists:flatten(io_lib:format("mustache_opts: ~ts", [Msg]));
format_error({bad_opt, Key, Value}) ->
    lists:flatten(io_lib:format("mustache_opts: bad value for ~p: ~p",
                                [Key, Value]));
format_error({bad_prefix, P}) ->
    lists:flatten(io_lib:format(
                    "mustache_opts: prefix ~p is not a legal start of an "
                    "Erlang atom (want [a-z][a-zA-Z0-9_]* or \"\")", [P]));
format_error({ext_not_loadable, M, R}) ->
    lists:flatten(io_lib:format(
                    "mustache_opts: extension module ~p could not be loaded "
